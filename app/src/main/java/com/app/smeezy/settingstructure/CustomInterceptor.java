...
/**
 * Created by kipl146 on 5/4/2016.
 */
public class CustomInterceptor implements Interceptor {

    private static String LOGTAG = CustomInterceptor.class.getName();
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (token != null && !token.isEmpty()) {
            Log.d("Builder", "common " + token);
            request.newBuilder()
                    .addHeader("content-type", "application/json");

        } else {
            Log.d("Builder", "not common ");
        ...
        if (logHeaders) {
            if (hasRequestBody) {
                // ActivityRequest body headers are only present when installed as a network interceptor. Force
